...
func (in *imageNoise) init() {
	...
	custom := easyga.GeneticAlgorithmFunctions{
		FitnessFunction: func(c *easyga.Chromosome) {
			...
			tempNoiseAmp := geneToPercent(c.Gene[:8]) * 30.
			tempNoiseFreqRow := geneToPercent(c.Gene[8:16]) * 0.01
			tempNoiseFreqCol := geneToPercent(c.Gene[16:24]) * 0.01

			c.Fitness = in.totalError(tempNoiseAmp, tempNoiseFreqRow, tempNoiseFreqCol)
